...
可以提前为`img`标签添加`width`和`height`属性值，如此一来在加载过程中也能具有稳定的宽高显示，从而保证页面的稳定。如果要实现固定的宽高比，自适应宽高比，则可以预设宽高值之一为固定值，另一个为`auto`。
...
对于需要针对图片加载做优化的场景，可以使用`懒加载`技术，让部分图片的`src`属性值为空，并且在某些特殊条件下设置正确的`src`值，让浏览器请求资源并且展示出来。
